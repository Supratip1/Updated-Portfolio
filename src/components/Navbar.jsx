  ...
  return (
    <div className={`fixed top-0 left-0 right-0 z-50 px-6 md:px-20 py-6 md:py-8 font-sans antialiased ${!isMenuOpen ? 'transition-all duration-300' : ''} ${
      ...
      <div className="flex items-center justify-between">
        ...
        {/* Pill-shaped MENU button for desktop, now at top right with social icons inside */}
        <div className="hidden md:flex items-center gap-4 ml-auto">
          <div className="relative">
            <button
              className="flex items-center gap-3 px-6 py-2 rounded-full bg-[#212121] text-white font-NueueMontreal text-lg font-semibold shadow-md hover:bg-[#333] transition-colors duration-200 focus:outline-none"
              onClick={toggleMenu}
            >
            ...
            {isMenuOpen && (
              <div className="absolute right-0 mt-3 w-64 bg-white rounded-2xl shadow-xl border border-zinc-200 z-50 flex flex-col py-3 animate-fadeIn">
                {['HOME', 'ABOUT US', 'PROJECTS', 'CONTACT'].map((label) => (
                  <a
                    key={label}
                    href={`#${label.replace(/ /g, '-').toLowerCase()}`}
                    onClick={e => handleNavClick(e, label)}
                    className="px-6 py-3 text-[#000] font-NueueMontreal text-[26px] font-medium uppercase tracking-wide hover:bg-zinc-100 transition-colors duration-150"
                    style={{ fontFamily: 'NeueMontreal, sans-serif', fontSize: '26px', letterSpacing: '0.02em' }}
                  >
                    {label}
                  </a>
                ))}
